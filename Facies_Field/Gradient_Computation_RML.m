...
elseif OPtimization_Algorithm==1
    ...
    for tt=1:ddd_welldata
        ...
        EE=EE+(Gra_BHP(:,:,tt))'*inv(Rp(tttt3,tttt3))*(BHPFD(:,tt)-BHP_RML(1+(IIJ-1)*nWI:IIJ*nWI,tt));
        EE=EE+(Gra_Flux(:,:,tt))'*inv(Rq(tttt4,tttt4))*(FluxFD(:,tt)-Flux_RML(1+(IIJ-1)*nWP:IIJ*nWP,tt));
        EE=EE+(Gra_Wat(:,:,tt))'*inv(Rfw(tttt4,tttt4))*(WatercutFD(:,tt)-Watercut_RML(1+(IIJ-1)*nWP:IIJ*nWP,tt));
