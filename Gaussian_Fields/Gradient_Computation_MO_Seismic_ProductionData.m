...
elseif OPtimization_Algorithm==1
    ...
    for tt=1:ddd_seismicdata
        ...
        EE=EE+weightings(JJI)*(Gra_Seismic2(:,:,tt))'*inv(Rs(tttt1,tttt1))*(SatFD(:,tt)-Sat(:,tt));
    ...
    for tt=1:ddd_welldata
        ...
        EE=EE+weightings(JJI)*(Gra_BHP2(:,:,tt))'*inv(Rp(tttt3,tttt3))*(BHPFD(:,tt)-BHP(:,tt));
        EE=EE+weightings(JJI)*(Gra_Flux2(:,:,tt))'*inv(Rq(tttt4,tttt4))*(FluxFD(:,tt)-Flux(:,tt));
        EE=EE+weightings(JJI)*(Gra_Wat2(:,:,tt))'*inv(Rfw(tttt4,tttt4))*(WatercutFD(:,tt)-Watercut(:,tt));
