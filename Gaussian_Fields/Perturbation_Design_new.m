...
if Vandermonde_Chebyshev>2
    ...
    XX=zeros(5*max(Num_LPCA)*nz,length(Num_LPCA)*max(Num_LPCA)*nz);
    EnK_num=max(Num_LPCA);
    for i=1:5
    ...
    Perturb_Matrix=zeros(5*EnK_num*nz,length(Num_LPCA)*max(Num_LPCA)*nz);
    for z=1:nz
        for j=1:Domain_Num
    ...
    XX=[];
    for i=1:length(Num_LPCA)
        XX=[XX Perturb_Matrix(:,1+(i-1)*max(Num_LPCA):Num_LPCA(i)+(i-1)*max(Num_LPCA))];
    end
    Perturb_Matrix=XX;


elseif Vandermonde_Chebyshev<3
%     
%     clear Perturb_Index Perturb_Matrix
% 
...
%     Perturb_Matrix=Perturb_Matrix/10;
    ...
    Perturb_Matrix=zeros(max(Num_LPCA)*nz,length(Num_LPCA)*max(Num_LPCA)*nz);
    for z=1:nz
        for j=1:Domain_Num
            tt1=1+(j-1)*max(Num_LPCA)+(z-1)*max(Num_LPCA)*Domain_Num:j*max(Num_LPCA)+(z-1)*max(Num_LPCA)*Domain_Num;
            tt2=1+(z-1)*max(Num_LPCA):z*max(Num_LPCA);
            Perturb_Matrix(tt2,tt1)=delta*eye(max(Num_LPCA),max(Num_LPCA));
        end
    end
